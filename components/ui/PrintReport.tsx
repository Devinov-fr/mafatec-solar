...
const ReportPDFPopup: React.FC<ReportPDFPopupProps> = ({
  ...
  const totalProduction = monthlyData.reduce((sum: number, m: any) => sum + (m.E_m || 0), 0);
  const totalIrradiation = monthlyData.reduce((sum: number, m: any) => sum + (m["H(i)_m"] || 0), 0);
  const maxProduction = monthlyData.length > 0 ? Math.max(...monthlyData.map((m: any) => m.E_m || 0)) : 0;
  const maxIrradiation = monthlyData.length > 0 ? Math.max(...monthlyData.map((m: any) => m["H(i)_m"] || 0)) : 0;
  const maxVariability = monthlyData.length > 0 ? Math.max(...monthlyData.map((m: any) => m.SD_m || 0)) : 0;
  ...
  const renderAreaChart = (data: any[], color: string, maxValue: number, height: number = 140) => {
    ...
    return (
      ...
      >
        ...
        {data.map((item, idx) => {
          ...
          return (
            <circle key={idx} cx={x} cy={y} r="4" fill={color} stroke="#fff" strokeWidth="2" />
          );
        })}
        ...
        <text x={-5} y={15} textAnchor="end" fontSize="8" fill="#a0a4b8" fontFamily="'Plus Jakarta Sans', sans-serif">
          {formatInteger(maxVal)}
        </text>
        <text x={-5} y={chartHeight + 10} textAnchor="end" fontSize="8" fill="#a0a4b8" fontFamily="'Plus Jakarta Sans', sans-serif">
          0
        </text>
      </svg>
    );
  ...
  const roofImageUrl = processedImageUrl || calepinageImage || panels.find((p: any) => p.imageUrl)?.imageUrl || "/toit-maison.jpg";
          ...
          <div ref={reportRef} style={{ maxWidth: "210mm", margin: "0 auto" }}>
            ...
            <div className="page" style={{ 
              ...
              <div style={{ 
                position: "absolute", 
                inset: 0, 
                background: "radial-gradient(ellipse 80% 70% at 15% 85%, #13162f 0%, rgba(19, 22, 47, 0.7) 40%, transparent 70%)" 
              }} />
              <div style={{ 
                position: "relative", 
                zIndex: 2, 
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: "9px", letterSpacing: "1.4px", textTransform: "uppercase", color: "#A82E12", padding: "4px 8px", borderRadius: "3px" }}>RGE</span>
                      <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: "9px", letterSpacing: "1.4px", textTransform: "uppercase", color: "#A82E12", padding: "4px 8px", borderRadius: "3px" }}>Qualifelec</span>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ padding: "8mm 0" }}>
                  ...
                  <p style={{ fontSize: "14px", lineHeight: 1.7, color: "rgba(243,239,230,0.62)", maxWidth: "450px" }}>
                    Estimation de production, irradiation et performance d'une installation photovoltaïque de {puissancePv} kWc, calculée selon les données d'irradiation officielles et les paramètres réels du site.
                  </p>
                ...
                <div style={{ 
                  ...
                  <div style={{ fontFamily: "'Spectral', Georgia, serif", fontStyle: "italic", fontSize: "11px", color: "#ffffff" }}>Préparé par <strong style={{ fontStyle: "normal", fontWeight: 600, color: "#ffffff" }}>MAFATEC</strong> — Énergie solaire</div>
                  <div style={{ fontSize: "8px", textAlign: "right", lineHeight: "1.5", letterSpacing: "0.5px" }}>
                    Réf. ÉTUDE PV · {puissancePv} kWc<br />
                    Édité le {date}
                  </div>
                </div>
              </div>
              ...
              <div style={{ 
                ...
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
                    ...
                    <span style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "2.5px", textTransform: "uppercase", color: "#c93b18" }}>Résultats de la simulation</span>
                  </div>
                  <h2 style={{ fontFamily: "'Spectral', Georgia, serif", fontSize: "24px", fontWeight: 600, color: "#15172b", marginBottom: "8px" }}>Les indicateurs <em style={{ fontStyle: "italic", color: "#c93b18" }}>clés</em> de production</h2>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px", marginBottom: "16px" }}>
                    <div style={{ border: "1px solid #e8e8ea", borderRadius: "12px", padding: "16px", background: "#fff" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
                        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#c93b18" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                          <path d="M4 5h16M4 12h16M4 19h10" />
                        </svg>
                        <h3 style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", color: "#c93b18", margin: 0 }}>
                          Entrées fournies
                        </h3>
                      </div>
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px" }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Latitude</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{clickedPosition.lat.toFixed(6)}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px" }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Longitude</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{clickedPosition.lng.toFixed(6)}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Horizon</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>Calculé</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>PV installée</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{puissancePv} kWc</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Pertes système</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{systemLosses} %</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Inclinaison</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{inclinaison}°</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Azimut</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{azimut}° ({getAzimuthDirection(parseFloat(azimut))})</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div style={{ border: "1px solid #e8e8ea", borderRadius: "12px", padding: "16px", background: "#fff" }}>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Angle d'incidence</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{formatNumber(data?.outputs?.totals?.fixed?.l_aoi || 0, 2)}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Effets spectraux</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{data?.outputs?.totals?.fixed?.l_spec || "0"}</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Température & faible irradiance</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{formatNumber(data?.outputs?.totals?.fixed?.l_tg || 0, 2)} %</span>
                        </div>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                          <span style={{ fontSize: "11px", color: "#454a63" }}>Pertes totales</span>
                          <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{formatNumber(data?.outputs?.totals?.fixed?.l_total || 0, 2)}</span>
                        </div>
                      </div>
                      ...
                      {voltageDropResult && voltageDropResult.vdrop && (
                        <>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                              <span style={{ fontSize: "11px", color: "#454a63" }}>Chute de tension</span>
                              <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{voltageDropResult.vdrop} V</span>
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", paddingBottom: "6px"  }}>
                              <span style={{ fontSize: "11px", color: "#454a63" }}>Pourcentage de chute</span>
                              <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{voltageDropResult.vdropPct} %</span>
                            </div>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
                              <span style={{ fontSize: "11px", color: "#454a63" }}>Résistance de fil</span>
                              <span style={{ fontSize: "11px", fontWeight: "bold", color: "#15172b" }}>{voltageDropResult.rwire} Ω</span>
                            </div>
                          </div>
                        </>
                      )}
                  ...
                  <div style={{ background: "#f5f5f7", padding: "16px", borderRadius: "12px", marginTop:"20px" }}>
                    ...
                    <p style={{ fontSize: "10px", color: "#454a63", lineHeight: 1.55 }}>Les estimations sont calculées à partir des données d'irradiation solaire de référence pour la localisation du site, en tenant compte de l'inclinaison, de l'azimut, des pertes système ({systemLosses}%) et du calcul automatique de l'horizon. Les valeurs de production constituent une estimation et peuvent varier selon les conditions météorologiques réelles.</p>
                  </div>
                </div>
              ...
              <div style={{ 
                ...
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
                    ...
                    <span style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "2.5px", textTransform: "uppercase", color: "#c93b18" }}>Calepinage</span>
                  </div>
                  <h2 style={{ fontFamily: "'Spectral', Georgia, serif", fontSize: "24px", fontWeight: 600, color: "#15172b", marginBottom: "8px" }}>Emplacement des <em style={{ fontStyle: "italic", color: "#c93b18" }}>panneaux</em></h2>
                  <p style={{ fontSize: "11px", color: "#7a7e95", marginTop: "4px", marginBottom: "20px" }}>Implantation et forme du champ photovoltaïque sur la toiture.</p>
                  ...
                  <div style={{ border: "1px solid #e8e8ea", borderRadius: "14px", padding: "20px", background: "#fff", marginBottom: "20px", textAlign: "center" }}>
                    {panels && panels.length > 0 && roofImageUrl ? (
                      ...
                      <div>
                        <svg viewBox="0 0 1024 730" preserveAspectRatio="xMidYMid meet" style={{ width: "100%", height: "auto", borderRadius: "8px", marginBottom: "16px" }}>
                          <image 
                            href="/toit-maison.jpg" 
                            x="0" y="0" width="1024" height="730" 
                      ...
                      <div style={{ background: "#f5f5f7", borderRadius: "8px", padding: "40px", textAlign: "center" }}>
                        <div style={{ fontSize: "14px", color: "#7a7e95", marginBottom: "8px" }}>📐 Aucun calepinage défini</div>
                        <div style={{ fontSize: "12px", color: "#a0a4b8" }}>Utilisez l'outil de calepinage pour définir l'emplacement des panneaux</div>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div style={{ 
                ...
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
                    ...
                    <span style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "2.5px", textTransform: "uppercase", color: "#c93b18" }}>Détail mensuel</span>
                  </div>
                  <h2 style={{ fontFamily: "'Spectral', Georgia, serif", fontSize: "24px", fontWeight: 600, color: "#15172b", marginBottom: "8px" }}>Énergie PV & irradiation <em style={{ fontStyle: "italic", color: "#c93b18" }}>mensuelle</em></h2>
                  <p style={{ fontSize: "11px", color: "#7a7e95", marginTop: "4px", marginBottom: "20px" }}>Production, irradiation et variabilité mois par mois, avec total annuel.</p>
                      ...
                      <tbody>
                        {monthlyData.map((m: any, i: number) => (
                          <tr 
                            key={i} 
                            style={{ 
                              borderBottom: i === monthlyData.length - 1 ? "none" : "1px solid #e8e8ea",
                          ...
                          >
                            ...
                            <td style={{ 
                              ...
                              textAlign: "center", 
                              fontWeight: "bold"
                            }}>{formatNumber(m.E_m || 0, 2)}</td>
                            <td style={{ 
                              padding: "10px 12px", 
                              textAlign: "center"
                            }}>{formatNumber(m["H(i)_m"] || 0, 2)}</td>
                            <td style={{ 
                              padding: "10px 12px", 
                              textAlign: "center"
                            }}>{formatNumber(m.SD_m || 0, 2)}</td>
                          </tr>
                        ))}
                        <tr style={{ 
                          background: "#13162f", 
                          ...
                          <td style={{ 
                            ...
                            fontWeight: "bold",
                            color: "#ffffff"
                          }}>{formatNumber(totalProduction, 2)}</td>
                          <td style={{ 
                            padding: "12px 12px", 
                            textAlign: "center", 
                            fontWeight: "bold",
                            color: "#ffffff"
                          }}>{formatNumber(totalIrradiation, 2)}</td>
                          <td style={{ 
                            padding: "12px 12px", 
                            textAlign: "center", 
                            fontWeight: "bold",
                            color: "#ffffff"
                          }}>{formatNumber(variabiliteAnnuelle, 1)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
                ...
                <div style={{ background: "#f5f5f7", padding: "16px", borderRadius: "12px", marginTop:"20px" }}>
                  <h4 style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "1.8px", textTransform: "uppercase", color: "#3a55b0", marginBottom: "6px" }}>Note</h4>
                  <p style={{ fontSize: "10px", color: "#454a63", lineHeight: 1.55 }}>Les valeurs mensuelles sont issues de la simulation d'irradiation pour le site et tiennent compte des pertes système. Le total annuel de production s'élève à {formatNumber(totalProduction, 2)} kWh.</p>
                </div>
              ...
              <div style={{ 
                ...
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
                    ...
                    <span style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "2.5px", textTransform: "uppercase", color: "#c93b18" }}>Évolution annuelle</span>
                  </div>
                  ...
                  <div style={{ border: "1px solid #e8e8ea", borderRadius: "12px", padding: "12px", marginBottom: "16px", background: "#fff" }}>
                    ...
                    {renderAreaChart(chartDataIrradiation, "#a8884a", maxIrradiation, 160)}
                  </div>

                  <div style={{ border: "1px solid #e8e8ea", borderRadius: "12px", padding: "20px", background: "#fff" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "16px" }}>
                      <div style={{ width: "12px", height: "12px", borderRadius: "50%", background: "#3a55b0" }} />
                      <span style={{ fontSize: "12px", fontWeight: "bold", color: "#15172b" }}>Variabilité mensuelle (kWh)</span>
                    </div>
                    {renderAreaChart(chartDataVariability, "#3a55b0", maxVariability, 160)}
                  </div>
                </div>
              ...
              <div style={{ 
                ...
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "10px" }}>
                    ...
                    <span style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "2.5px", textTransform: "uppercase", color: "#c93b18" }}>Ensoleillement</span>
                  </div>
                  ...
                  <div style={{ padding: "0px", background: "#fff", marginBottom: "10px", display: "flex", justifyContent: "center", alignItems: "center", minHeight: "320px" }}>
                    <div style={{ transform: "scale(0.75)", transformOrigin: "center" }}>
                      ...
                      {(String(data?.inputs?.location?.latitude).startsWith("51.") || (data?.inputs?.location?.latitude || 0) > 51) && (
